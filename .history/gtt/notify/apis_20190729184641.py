...
class MarkAllAsDeleted(APIView):
    def post(self, request):
        user = User.objects.get(email=request.user)
        notifications = Notification.objects..filter(Q(actor__pk=user.id)|Q(recipient__pk=user.id)).delete()

class Delete(APIView):
    def post(self, request, slug):
        user = User.objects.get(email=request.user)
        notification_id = slug2id(slug)
        Notification.objects.filter(pk=notification_id, Q(actor__pk=user.id)|Q(recipient__pk=user.id))

class UnreadCount(APIView):
    def get(self, request):
        user = User.objects.get(email=request.user)
        notifications = Notification.objects.filter(Q(actor__pk=user.id)|Q(recipient__pk=user.id)).unread().count()

class AllCount(APIView):
    def get(self, request):
        user = User.objects.get(email=request.user)
        notifications = Notification.objects.filter()Q(actor__pk=user.id)|Q(recipient__pk=user.id).count()
