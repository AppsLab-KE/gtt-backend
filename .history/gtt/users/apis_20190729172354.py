...
class Reqpermission_classes = [IsAuthenticated]
    def post(self, request):
        user = User.objects.get(username=username)
        superusers = User.objects.filter(is_superuser=True)
        notify.send(actor=user, recepient=superusers, verb='make_writer', description="{} wants to become a writer.".format(user.usename))
