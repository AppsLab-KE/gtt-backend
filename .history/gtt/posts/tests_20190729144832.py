...
class PostsTest(APITestCase):
    ...
    def test_posts(self):
        ...
        print(os.path.join(settings.BASE_DIR, tag_posts_response.json()[0]['post_heading_image'].replace(settings.DOMAIN_URL, '')))
        os.remove(os.path.join(settings.BASE_DIR, tag_posts_response.json()[0]['post_heading_image'].replace(settings.DOMAIN_URL, '')))
    ...
    def test_ratings(self):
        ...
        tag_posts_response = self.client.get(reverse('posts:view_tag_posts', kwargs={'tag_name': 'UX'}), **auth_headers)
        self.assertEquals(tag_posts_response.status_code, status.HTTP_200_OK)
        view_post_response = self.client.get(reverse('posts:view_post', kwargs={'username': tag_posts_response.json()[0]['post_author']['username'], 'slug': tag_posts_response.json()[0]['slug']}), **auth_headers)
        self.assertEquals(view_post_response.status_code, status.HTTP_200_OK)
        like_post_response = self.client.post(reverse('posts:rate_post', kwargs={'slug': tag_posts_response.json()[0]['slug']}), data={'rating': 1}, **auth_headers)
        self.assertEquals(like_post_response.status_code, status.HTTP_200_OK)
        rated_posts_response = self.client.get(reverse('posts:view_rated_posts'), **auth_headers)
        self.assertEquals(rated_posts_response.status_code, status.HTTP_200_OK)
        dislike_post_response = self.client.post(reverse('posts:rate_post', kwargs={'slug': tag_posts_response.json()[0]['slug']}), data={'rating': 0}, **auth_headers)
    ...
    def test_comments(self):
        ...
        view_post_comments_response = self.client.get(reverse('posts:view_comments', kwargs={'slug': tag_posts_response.json()[0]['slug']}), **auth_headers)
    ...
    def test_replies(self):
        ...
        view_post_replies_response = self.client.get(reverse('posts:view_replies', kwargs={'resource_key': view_post_comments_response.json()[0]['resource_key']}), **auth_headers)
        self.assertEquals(view_post_replies_response.status_code, status.HTTP_200_OK)
        update_reply_response = self.client.post(reverse('posts:update_reply', kwargs={'resource_key': view_post_replies_response.json()[0]['resource_key']}), data={'reply': 'This is an updated test reply.'}, **auth_headers)
        self.assertEquals(update_reply_response.status_code, status.HTTP_200_OK)
        delete_reply_response = self.client.post(reverse('posts:delete_reply', kwargs={'resource_key': view_post_replies_response.json()[0]['resource_key']}), **auth_headers)
