...
class PostsTest(TestCase):
    ...
    def test_comments(self):
        ...
        tag_posts_response = self.client.get(settings.DOMAIN_URL + reverse('posts:view_tag_posts', kwargs={'tag_name': 'UX'}))
        self.assertEquals(tag_posts_response.status_code, '200')
        post_comment_response = self.client.post(settings.DOMAIN_URL + reverse('posts:create_comment', kwargs={'slug': tag_posts_response.json()[0]['slug']}), data={'comment': 'This is a test comment.'})
        self.assertEquals(post_comment_response.status_code, '200')
        ...
        update_post_comment_response = self.client.post(settings.DOMAIN_URL + reverse('posts:update_comment', kwargs={'resource_key': view_post_comments_response.json()[0]['slug']}), data={'comment': 'This is an updated test comment.'})
        self.assertEquals(update_post_comment_response.status_code, '200')
        delete_post_comment_response = self.client.post(settings.DOMAIN_URL + reverse('posts:delete_comment', kwargs={'resource_key': view_post_comments_response.json()[0]['slug']}), data={'comment': 'This is an updated test comment.'})
        self.assertEquals(update_post_comment_response.status_code, '200')
