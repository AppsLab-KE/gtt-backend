...
class Comment(APIView):
    def post(self, request, slug):
        ...
        try:
            post = Post.objects.get(slug=slug)
            comment, created = Comment.objects.update_or_create(commented_post=post, user_that_commented=user, defaults={'comment' : comment})
            if created:
                return Response({
                ...
                return Response({
                        "message": "You updated your comment for this post.",
                    })
        except Post.DoesNotExist:
            return Response({
                "message": "Post was not found.",
