...
class ViewRatedPosts(APIView):
    ...
    def get(self, request):
        ...
        if 'offset' in request.data and 'limit' in request.data:
            ...
            user_rated_posts = Post.objects.filter(user_that_rated__pk=user.id, ratings__rating=True)[offset:offset+limit]
        else:
            user_rated_posts = Post.objects.filter(user_that_rated__pk=user.id, ratings__rating=True)
        response_list = list()
