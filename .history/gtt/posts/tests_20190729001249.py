...
class PostsTest(APITestCase):
    ...
    def test_posts(self):
        ...
        tag_posts_response = self.client.get(settings.DOMAIN_URL + reverse('posts:view_tag_posts', kwargs={'tag_name': 'UX'}), **auth_headers)
        self.assertEquals(tag_posts_response.status_code, '200')
        update_post_response = self.client.post(settings.DOMAIN_URL + reverse('posts:update_post', kwargs={'slug': tag_posts_response.json()[0]['slug']}), data=self.update_post_data, **auth_headers)
        self.assertEquals(update_post_response.status_code, '200')
        delete_post_response = self.client.post(settings.DOMAIN_URL + reverse('posts:delete_post', kwargs={'slug': tag_posts_response.json()[0]['slug']}), **auth_headers)
