...
class PostsTest(TestCase):
    ...
    def test_posts(self):
        create_post_response = self.client.post(reverse('create_post'), data=self.create_post_data, **auth_headers)
        tag_posts_response = self.client.get(reverse('view_tag_posts', kwargs={'tag_name': 'UX'}), **auth_headers)
        print(tag_posts_response.json())
        get_post_response = self.client.get(reverse('view_post', kwargs={'username':, 'slug': }), **auth_headers)
        update_post_response = self.client.post(reverse('update_post', kwargs={'slug':}), data=self.update_post_data, **auth_headers)
        delete_post_response = self.client.post(reverse('delete_post', kwargs={'slug': }), **auth_headers)
