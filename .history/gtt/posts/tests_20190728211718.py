...
class PostsTest(TestCase):
    ...
    def test_posts(self):
        ...
        update_post_response = self.client.post(settings.DOMAIN_URL + reverse('posts:update_post', kwargs={''}), data=self.update_post_data)
        self.assertEquals(update_post_response.status_code, '200')
        delete_post_response = self.client.post(settings.DOMAIN_URL + reverse('posts:delete_post', kwargs={''}))
        self.assertEquals(update_post_response.status_code, '200')
