...
class PostsTest(TestCase):
    ...
    def test_comments(self):
        ...
        update_post_comment_response = self.client.post(settings.DOMAIN_URL + reverse('posts:update_comment', kwargs={'slug': tag_posts_response.json()[0]['slug']}), data={'comment': 'This is an updated test comment.'})
        self.assertEquals(update_post_comment_response.status_code, '200')
        delete_post_comment_response = self.client.post(settings.DOMAIN_URL + reverse('posts:delete_comment', kwargs={'slug': tag_posts_response.json()[0]['slug']}), data={'comment': 'This is an updated test comment.'})
        self.assertEquals(update_post_comment_response.status_code, '200')
